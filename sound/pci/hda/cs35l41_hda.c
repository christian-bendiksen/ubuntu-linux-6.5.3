...
static const struct reg_sequence cs35l41_hda_config[] = {
	{ CS35L41_PLL_CLK_CTRL,
	  0x00000430 }, // 3072000Hz, BCLK Input, PLL_REFCLK_EN = 1
	{ CS35L41_DSP_CLK_CTRL, 0x00000003 }, // DSP CLK EN
	{ CS35L41_GLOBAL_CLK_CTRL, 0x00000003 }, // GLOBAL_FS = 48 kHz
	{ CS35L41_SP_ENABLES, 0x00010000 }, // ASP_RX1_EN = 1
	{ CS35L41_SP_RATE_CTRL, 0x00000021 }, // ASP_BCLK_FREQ = 3.072 MHz
	{ CS35L41_SP_FORMAT,
	  0x20200200 }, // 32 bits RX/TX slots, I2S, clk consumer
...
static const struct reg_sequence cs35l41_hda_unmute[] = {
	{ CS35L41_AMP_DIG_VOL_CTRL,
	  0x00008000 }, // AMP_HPF_PCM_EN = 1, AMP_VOL_PCM  0.0 dB
...
static const struct reg_sequence cs35l41_hda_unmute_dsp[] = {
	...
	{ CS35L41_AMP_GAIN_CTRL,
	  0x00000233 }, // AMP_GAIN_PCM = 17.5dB AMP_GAIN_PDM = 19.5dB
};

static const struct reg_sequence cs35l41_hda_mute[] = {
	{ CS35L41_AMP_GAIN_CTRL, 0x00000000 }, // AMP_GAIN_PCM 0.5 dB
	{ CS35L41_AMP_DIG_VOL_CTRL,
	  0x0000A678 }, // AMP_HPF_PCM_EN = 1, AMP_VOL_PCM Mute
};
					 ...
					 int spkid, const char *filetype)
{
	...
	if (spkid > -1 && ssid && amp_name)
		*filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s-spkid%d-%s.%s",
				      dir, CS35L41_PART, dsp_name,
				      hda_cs_dsp_fw_ids[cs35l41->firmware_type],
				      ssid, spkid, amp_name, filetype);
	else if (spkid > -1 && ssid)
		*filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s-spkid%d.%s",
				      dir, CS35L41_PART, dsp_name,
				      hda_cs_dsp_fw_ids[cs35l41->firmware_type],
				      ssid, spkid, filetype);
	else if (ssid && amp_name)
		*filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s-%s.%s", dir,
				      CS35L41_PART, dsp_name,
				      hda_cs_dsp_fw_ids[cs35l41->firmware_type],
				      ssid, amp_name, filetype);
	else if (ssid)
		*filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s-%s.%s", dir,
				      CS35L41_PART, dsp_name,
				      hda_cs_dsp_fw_ids[cs35l41->firmware_type],
				      ssid, filetype);
	else
		*filename = kasprintf(GFP_KERNEL, "%s%s-%s-%s.%s", dir,
				      CS35L41_PART, dsp_name,
				      hda_cs_dsp_fw_ids[cs35l41->firmware_type],
				      filetype);
...
static int cs35l41_request_firmware_files_spkid(
	struct cs35l41_hda *cs35l41, const struct firmware **wmfw_firmware,
	char **wmfw_filename, const struct firmware **coeff_firmware,
	char **coeff_filename)
{
	int ret;

	/* try cirrus/part-dspN-fwtype-sub<-spkidN><-ampname>.wmfw */
	ret = cs35l41_request_firmware_file(
		cs35l41, wmfw_firmware, wmfw_filename, CS35L41_FIRMWARE_ROOT,
		cs35l41->acpi_subsystem_id, cs35l41->amp_name,
		cs35l41->speaker_id, "wmfw");
	if (!ret) {
		/* try cirrus/part-dspN-fwtype-sub<-spkidN><-ampname>.bin */
		return cs35l41_request_firmware_file(
			cs35l41, coeff_firmware, coeff_filename,
	...
	ret = cs35l41_request_firmware_file(
		cs35l41, wmfw_firmware, wmfw_filename, CS35L41_FIRMWARE_ROOT,
		cs35l41->acpi_subsystem_id, cs35l41->amp_name, -1, "wmfw");
	if (!ret) {
		/* try cirrus/part-dspN-fwtype-sub<-spkidN><-ampname>.bin */
		return cs35l41_request_firmware_file(
			cs35l41, coeff_firmware, coeff_filename,
	...
	ret = cs35l41_request_firmware_file(
		cs35l41, wmfw_firmware, wmfw_filename, CS35L41_FIRMWARE_ROOT,
		cs35l41->acpi_subsystem_id, NULL, cs35l41->speaker_id, "wmfw");
	if (!ret) {
		/* try cirrus/part-dspN-fwtype-sub<-spkidN><-ampname>.bin */
		ret = cs35l41_request_firmware_file(
			cs35l41, coeff_firmware, coeff_filename,
	...
	ret = cs35l41_request_firmware_file(
		cs35l41, wmfw_firmware, wmfw_filename, CS35L41_FIRMWARE_ROOT,
		cs35l41->acpi_subsystem_id, NULL, -1, "wmfw");
	if (!ret) {
		/* try cirrus/part-dspN-fwtype-sub<-spkidN><-ampname>.bin */
		ret = cs35l41_request_firmware_file(
			cs35l41, coeff_firmware, coeff_filename,
...
static int cs35l41_request_firmware_files(
	struct cs35l41_hda *cs35l41, const struct firmware **wmfw_firmware,
	char **wmfw_filename, const struct firmware **coeff_firmware,
	char **coeff_filename)
{
	...
	if (cs35l41->speaker_id > -1) {
		ret = cs35l41_request_firmware_files_spkid(
			cs35l41, wmfw_firmware, wmfw_filename, coeff_firmware,
			coeff_filename);
		goto out;
	}
	...
	if (!ret) {
		...
		if (ret)
			...
			ret = cs35l41_request_firmware_file(
				cs35l41, coeff_firmware, coeff_filename,
				CS35L41_FIRMWARE_ROOT,
				cs35l41->acpi_subsystem_id, NULL, -1, "bin");
	}
...
out:
	...
	ret = cs35l41_request_firmware_file(cs35l41, wmfw_firmware,
					    wmfw_filename,
					    CS35L41_FIRMWARE_ROOT, NULL, NULL,
					    -1, "wmfw");
	if (!ret)
		/* fallback try cirrus/part-dspN-fwtype.bin */
		ret = cs35l41_request_firmware_file(cs35l41, coeff_firmware,
						    coeff_filename,
						    CS35L41_FIRMWARE_ROOT, NULL,
						    NULL, -1, "bin");
...
static int cs35l41_apply_calibration(struct cs35l41_hda *cs35l41,
				     __be32 ambient, __be32 r0, __be32 status,
				     __be32 checksum)
{
	int ret;

	ret = hda_cs_dsp_write_ctl(&cs35l41->cs_dsp, CAL_AMBIENT_DSP_CTL_NAME,
				   CAL_DSP_CTL_TYPE, CAL_DSP_CTL_ALG, &ambient,
				   4);
	if (ret) {
		dev_err(cs35l41->dev, "Cannot Write Control: %s - %d\n",
			CAL_AMBIENT_DSP_CTL_NAME, ret);
		return ret;
	}
	ret = hda_cs_dsp_write_ctl(&cs35l41->cs_dsp, CAL_R_DSP_CTL_NAME,
				   CAL_DSP_CTL_TYPE, CAL_DSP_CTL_ALG, &r0, 4);
	if (ret) {
		dev_err(cs35l41->dev, "Cannot Write Control: %s - %d\n",
			CAL_R_DSP_CTL_NAME, ret);
		return ret;
	}
	ret = hda_cs_dsp_write_ctl(&cs35l41->cs_dsp, CAL_STATUS_DSP_CTL_NAME,
				   CAL_DSP_CTL_TYPE, CAL_DSP_CTL_ALG, &status,
				   4);
	if (ret) {
		dev_err(cs35l41->dev, "Cannot Write Control: %s - %d\n",
			CAL_STATUS_DSP_CTL_NAME, ret);
		return ret;
	}
	ret = hda_cs_dsp_write_ctl(&cs35l41->cs_dsp, CAL_CHECKSUM_DSP_CTL_NAME,
				   CAL_DSP_CTL_TYPE, CAL_DSP_CTL_ALG, &checksum,
				   4);
	if (ret) {
		dev_err(cs35l41->dev, "Cannot Write Control: %s - %d\n",
			CAL_CHECKSUM_DSP_CTL_NAME, ret);
		return ret;
	}
...
{
	static efi_guid_t efi_guid = EFI_GUID(0x02f9af02, 0x7734, 0x4233, 0xb4,
					      0x3d, 0x93, 0xfe, 0x5a, 0xa3,
					      0x5d, 0xb3);
		...
		if (status == EFI_SUCCESS) {
			...
			if (efi_data->count > cs35l41->index) {
				...
				dev_dbg(cs35l41->dev,
					...
					cl->calAmbient, cl->calStatus,
					cl->calR);

				/* Calibration can only be applied whilst the DSP is not running */
				ret = cs35l41_apply_calibration(
					cs35l41, cpu_to_be32(cl->calAmbient),
					cpu_to_be32(cl->calR),
...
static int cs35l41_save_calibration(struct cs35l41_hda *cs35l41)
{
	dev_warn(cs35l41->dev,
		 "Calibration not supported without EFI support.\n");
	return 0;
}
#endif
...
static int cs35l41_init_dsp(struct cs35l41_hda *cs35l41)
{
	...
	ret = cs_dsp_power_up(dsp, wmfw_firmware, wmfw_filename, coeff_firmware,
			      coeff_filename,
			      hda_cs_dsp_fw_ids[cs35l41->firmware_type]);
	if (ret)
		goto err_release;
...
static void cs35l41_error_release(struct device *dev, struct regmap *regmap,
				  unsigned int mask)
{
...
{
	cs35l41_error_release(cs35l41->dev, cs35l41->regmap,
			      cs35l41->irq_errors);
...
static void cs35l41_hda_play_start(struct device *dev)
{
	...
	if (cs35l41->firmware_running) {
		...
		regmap_update_bits(reg, CS35L41_PWR_CTRL2,
				   ...
				   1 << CS35L41_VMON_EN_SHIFT |
					   1 << CS35L41_IMON_EN_SHIFT);
		cs35l41_set_cspl_mbox_cmd(cs35l41->dev, reg,
					  CSPL_MBOX_CMD_RESUME);
	} else {
		regmap_multi_reg_write(reg, cs35l41_hda_config,
				       ARRAY_SIZE(cs35l41_hda_config));
	}
	regmap_update_bits(reg, CS35L41_PWR_CTRL2, CS35L41_AMP_EN_MASK,
			   1 << CS35L41_AMP_EN_SHIFT);
...
static void cs35l41_hda_pause_start(struct device *dev)
{
	...
	regmap_multi_reg_write(reg, cs35l41_hda_mute,
...
static void cs35l41_hda_playback_hook(struct device *dev, int action)
{
	...
	case HDA_GEN_PCM_ACT_CLOSE:
		...
		if (!cs35l41->firmware_running && cs35l41->request_fw_load &&
			...
			dev_info(
				dev,
				"Requesting Firmware Load after HDA_GEN_PCM_ACT_CLOSE\n");
...
static int cs35l41_hda_channel_map(struct device *dev, unsigned int tx_num,
				   ...
				   unsigned int *rx_slot)
{
	...
	if (!cs35l41->amp_name) {
		...
		cs35l41->amp_name = devm_kasprintf(cs35l41->dev, GFP_KERNEL,
						   "%s%d",
						   channel_name[*rx_slot],
						   cs35l41->channel_index);
		if (!cs35l41->amp_name)
			return -ENOMEM;
	}

	return cs35l41_set_channels(cs35l41->dev, cs35l41->regmap, tx_num,
...
static int cs35l41_ready_for_reset(struct cs35l41_hda *cs35l41)
{
	...
	if (cs35l41->firmware_running) {
		...
		if (cs35l41->hw_cfg.bst_type == CS35L41_EXT_BOOST)
			cs35l41_init_boost(cs35l41->dev, cs35l41->regmap,
					   &cs35l41->hw_cfg);
...
static int cs35l41_system_suspend(struct device *dev)
{
	...
	if (ret) {
		dev_err(dev,
			"System Suspend Failed, unable to runtime suspend: %d\n",
			ret);
		return ret;
	}
...
static int cs35l41_system_resume(struct device *dev)
{
	...
	if (ret) {
		dev_err(dev,
			"System Resume Failed: Unable to runtime resume: %d\n",
			ret);
		return ret;
	}
...
static int cs35l41_smart_amp(struct cs35l41_hda *cs35l41)
{
	...
	if (ret) {
		dev_warn(cs35l41->dev,
			 "Cannot Initialize Firmware. Error: %d\n", ret);
		goto clean_dsp;
	}
	...
	ret = read_poll_timeout(hda_cs_dsp_read_ctl, ret,
				...
				1000, 15000, false, &cs35l41->cs_dsp,
				HALO_STATE_DSP_CTL_NAME,
				HALO_STATE_DSP_CTL_TYPE, HALO_STATE_DSP_CTL_ALG,
				&halo_sts, sizeof(halo_sts));

	if (ret) {
		dev_err(cs35l41->dev,
			"Timeout waiting for HALO Core to start. State: %u\n",
			halo_sts);
		goto clean_dsp;
	}
...
static void cs35l41_fw_load_work(struct work_struct *work)
{
	struct cs35l41_hda *cs35l41 =
		container_of(work, struct cs35l41_hda, fw_load_work);
	...
	if (cs35l41->playback_started)
		dev_err(cs35l41->dev,
			"Cannot Load/Unload firmware during Playback. Retrying...\n");
	else
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->playback_started) {
		dev_err(cs35l41->dev,
			"Cannot Load/Unload firmware during Playback\n");
		return -EBUSY;
	}
...
{
	return snd_ctl_enum_info(uinfo, 1, ARRAY_SIZE(hda_cs_dsp_fw_ids),
				 hda_cs_dsp_fw_ids);
}
...
static int cs35l41_create_controls(struct cs35l41_hda *cs35l41)
{
	...
	scnprintf(fw_type_ctl_name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN,
		  "%s DSP1 Firmware Type", cs35l41->amp_name);
	scnprintf(fw_load_ctl_name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN,
		  "%s DSP1 Firmware Load", cs35l41->amp_name);

	ret = snd_ctl_add(cs35l41->codec->card,
			  snd_ctl_new1(&fw_type_ctl, cs35l41));
	if (ret) {
	...
	ret = snd_ctl_add(cs35l41->codec->card,
			  snd_ctl_new1(&fw_load_ctl, cs35l41));
	if (ret) {
		dev_err(cs35l41->dev, "Failed to add KControl %s = %d\n",
...
static int cs35l41_hda_bind(struct device *dev, struct device *master,
			    void *master_data)
{
	struct cs35l41_hda *cs35l41 = dev_get_drvdata(dev);
	...
	if (!comps || cs35l41->index < 0 ||
	    cs35l41->index >= HDA_MAX_COMPONENTS)
		return -EINVAL;
	...
	if (!cs35l41->acpi_subsystem_id)
		cs35l41->acpi_subsystem_id = kasprintf(
			GFP_KERNEL, "%.8x", comps->codec->core.subsystem_id);
	...
	if (!device_link_add(&comps->codec->core.dev, cs35l41->dev,
			     DL_FLAG_STATELESS))
...
static void cs35l41_hda_unbind(struct device *dev, struct device *master,
			       void *master_data)
{
	struct cs35l41_hda *cs35l41 = dev_get_drvdata(dev);
	struct hda_component *comps = master_data;
...
static const struct cs35l41_irq cs35l41_irqs[] = {
	CS35L41_IRQ(BST_OVP_ERR, "Boost Overvoltage Error",
		    cs35l41_bst_ovp_err),
	CS35L41_IRQ(BST_DCM_UVP_ERR, "Boost Undervoltage Error",
		    cs35l41_bst_dcm_uvp_err),
	CS35L41_IRQ(BST_SHORT_ERR, "Boost Inductor Short Error",
...
static int cs35l41_hda_apply_properties(struct cs35l41_hda *cs35l41)
{
	...
	if (cs35l41->irq && using_irq) {
		ret = devm_regmap_add_irq_chip(
			cs35l41->dev, cs35l41->regmap, cs35l41->irq,
			IRQF_ONESHOT | IRQF_SHARED | irq_pol, 0,
			&cs35l41_regmap_irq_chip, &cs35l41->irq_data);
		if (ret)
			return ret;

		for (i = 0; i < ARRAY_SIZE(cs35l41_irqs); i++) {
			irq = regmap_irq_get_virq(cs35l41->irq_data,
						  cs35l41_irqs[i].irq);
			if (irq < 0)
				return irq;

			ret = devm_request_threaded_irq(
				cs35l41->dev, irq, NULL,
	...
	return cs35l41_hda_channel_map(cs35l41->dev, 0, NULL, 1,
				       &hw_cfg->spk_pos);
}
				  ...
				  int num_amps, int fixed_gpio_id)
{
	...
	if (fixed_gpio_id >= 0) {
		dev_dbg(dev, "Found Fixed Speaker ID GPIO (index = %d)\n",
			fixed_gpio_id);
		speaker_id_desc =
			gpiod_get_index(dev, NULL, fixed_gpio_id, GPIOD_IN);
...
static int cs35l41_hda_read_acpi(struct cs35l41_hda *cs35l41, const char *hid,
				 int id)
{
	struct cs35l41_hw_cfg *hw_cfg = &cs35l41->hw_cfg;
	 ...
	 */
	cs35l41->reset_gpio = fwnode_gpiod_get_index(acpi_fwnode_handle(adev),
						     "reset", cs35l41->index,
						     GPIOD_OUT_LOW,
						     "cs35l41-reset");
	...
	cs35l41->speaker_id =
		cs35l41_get_speaker_id(physdev, cs35l41->index, nval, -1);
		      ...
		      int irq, struct regmap *regmap)
{
	...
	ret = regmap_read_poll_timeout(cs35l41->regmap, CS35L41_IRQ1_STATUS4,
				       int_status,
				       int_status & CS35L41_OTP_BOOT_DONE, 1000,
				       100000);
	if (ret) {
	...
	ret = cs35l41_register_errata_patch(cs35l41->dev, cs35l41->regmap,
					    reg_revid);
	if (ret)
		goto err;
...
MODULE_AUTHOR(
	"Lucas Tanure, Cirrus Logic Inc, <tanureal@opensource.cirrus.com>");
MODULE_LICENSE("GPL");
